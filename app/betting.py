...
class Bettor:
    ...
    def ats_bets(self, ):
        odds = odds_helper.get_fanduel_odds()
    ...
    def all_bets(self, pot):
        ats_odds = odds_helper.get_fanduel_odds(future_days=1)
        h2h_odds = odds_helper.get_fanduel_odds(future_days=1, bet_type='h2h')
        o_u_odds = odds_helper.get_fanduel_ou_odds(future_days=1)

        if not ats_odds and not h2h_odds:
            return
        ...
        for game in ats_odds:
            ...
            home_team = self.config.get('name_map').get(home_team, home_team)
            away_team = self.config.get('name_map').get(away_team, away_team)
...
def print_bet_table(df, good_bets=True):
    columns = ['Num', 'Team', 'Spread', 'O/U', 'Opponent', 'American Odds', 'Probability', 'Payout', 'Model Chance',
               'Push Chance', 'Bet Percent', 'Bet Amount', 'Expected Value', 'Expected Return', 'Expected Profit',
               'To Win']
    ...
    for index, row in df.iterrows():
        ...
        for col in columns[1:]:
            ...
            elif col == 'Probability' or col == 'Model Chance' or col == 'Push Chance' or col == 'Bet Percent':
